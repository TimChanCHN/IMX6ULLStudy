...
unsigned short lcd_read_panelid(void)
{
    ...
    index = (unsigned char)gpio_pinread(GPIO3, 28);             /* M0 */
    index |= (unsigned char)gpio_pinread(GPIO3, 20) << 1;             /* M1 */
    index |= (unsigned char)gpio_pinread(GPIO3, 12) << 2;             /* M2 */

    if( index == 0 ) return ATK4342;
    else if( index == 1 ) return ATK7084;
    else if( index == 2 ) return ATK7016;
...
/* 画粗点，4个像素 */
inline void lcd_drawbigpoint(unsigned short x,unsigned short y,unsigned int color)
{
	lcd_drawpoint(x, y, color);
	...
	if( (x < tftlcd_dev.width)&&(y < tftlcd_dev.height) )
