...
AT_NONCACHEABLE_SECTION_ALIGN(sdma_buffer_descriptor_t bd_bk[BD_QUEUE_SIZE], 4) = {{0}};
...
int main(void)
{
    ...
    for (i = 0; i < BD_QUEUE_SIZE - 1; i++)
    {
        SDMA_ConfigBufferDescriptor(&bd[i], (uint32_t)&srcAddr[i*BUFFER_LEN], (uint32_t)&destAddr[i*BUFFER_LEN], kSDMA_TransferSize4Bytes,
                                    sizeof(uint32_t)*BUFFER_LEN, false, false, false, kSDMA_MemoryToMemory);
    }
    SDMA_ConfigBufferDescriptor(&bd[i], (uint32_t)&srcAddr[i*BUFFER_LEN], (uint32_t)&destAddr[i*BUFFER_LEN], kSDMA_TransferSize4Bytes,
                                sizeof(uint32_t)*BUFFER_LEN, true, true, true, kSDMA_MemoryToMemory);
    memcpy(bd_bk, bd, sizeof(bd));
